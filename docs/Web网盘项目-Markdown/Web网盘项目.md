...
> 当前仓库实现说明（2026-08-13）：本项目没有引入下文示例中的 ppconsul，而是复用已编译的 Workflow
> `WFConsulClient`。macOS 开发环境运行单节点 Consul 2.0.3；用户和文件 RPC 服务已经支持启动后注册、TCP
> 健康检查和停止前注销；网关会查询 passing 实例、转换端点、简单轮询，并按所选地址创建本次 sRPC 调用。实际
> Docker 命令、配置和验证方式以项目根目录 `README.md` 为准。
