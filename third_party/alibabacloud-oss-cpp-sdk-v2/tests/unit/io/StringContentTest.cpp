...
TEST(StringContentTest, Constructor) {
    auto content = std::make_shared<StringContent>("");
    EXPECT_EQ(0, content->length().value());
    EXPECT_FALSE(content->isOneShot());
    EXPECT_EQ(std::nullopt, content->path());

    content = std::make_shared<StringContent>("hello world");
    EXPECT_EQ(11, content->length().value());
    EXPECT_FALSE(content->isOneShot());
    EXPECT_EQ(std::nullopt, content->path());
}

TEST(StringContentTest, SpanSource) {
...
TEST(StringContentTest, SpanSourceState) {
    const std::string data = "hello world";
    auto content = std::make_shared<StringContent>(data);
    EXPECT_EQ(11, content->length().value());
    EXPECT_FALSE(content->isOneShot());
...
TEST(StringContentTest, SpanSourceRandom) {
    ...
    // move
    std::string cpData = data;
    content = std::make_shared<StringContent>(std::move(cpData));
    source = content->spanSource();
    for (i = 0; i < len;) {
        size_t remains = std::rand() % 16;
        remains = std::min(remains, len - i);
    ...
    content = std::make_shared<StringContent>(cosntData);
    source = content->spanSource();
    for (i = 0; i < len;) {
...
TEST(StringContentTest, SpanSourceNullptr) {
    const std::string data = "hello world";
    auto content = std::make_shared<StringContent>(data);
    EXPECT_EQ(11, content->length().value());
    EXPECT_FALSE(content->isOneShot());
