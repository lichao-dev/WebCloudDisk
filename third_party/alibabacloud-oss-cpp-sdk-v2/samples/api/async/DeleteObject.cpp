// Demonstrates: Async delete a single object.
...
int main(int argc, char* argv[]) {
    ...
    // --- Callback pattern ---
    {
        ...
        client->deleteObjectAsync(
            oss::models::DeleteObjectRequest()
                ...
                .setKey(args.key),
            oss::DeleteObjectAsyncCallback([&](oss::DeleteObjectOutcome outcome) {
                if (!outcome.has_value()) {
                    auto& e = outcome.error();
                    std::cerr << "DeleteObject(callback) fail"
                              << ", code: " << e.getCode()
                    ...
                    std::cout << "DeleteObject(callback) done"
    ...
    // --- Future pattern (asyncCall) ---
    {
        ...
        if (!outcome.has_value()) {
            auto& e = outcome.error();
            std::cerr << "DeleteObject(future) fail"
                      << ", code: " << e.getCode()
                      << ", message: " << e.getMessage()
                      << ", ec: " << e.getEC()
                      << ", requestId: " << e.getRequestId()
        ...
        auto& result = outcome.value();
        std::cout << "DeleteObject(future) done"
                  << ", status: " << result.getStatusCode()
                  << ", requestId: " << result.getRequestId()
                  << ", versionId: " << result.getVersionId()
                  << ", deleteMarker: " << result.getDeleteMarker() << std::endl;
    }
